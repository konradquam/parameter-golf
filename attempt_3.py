...
Attempt 3 notes:
- Builds on attempt 2.
- Keeps the same increasing-count global-head schedule.
- Gives different local heads different context sizes within each layer by doubling the window per head and per layer until they effectively become full-context.
"""
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 logit_chunk_tokens: int, logit_softcap: float, rope_base: float, partial_rope_dim: int, local_attention_window: int, train_seq_len: int, tied_embed_init_std: float,
                 qk_gain_init: float):
        super().__init__()
        ...
        self.blocks = [
            Block(
                ...
                rope_base,
                partial_rope_dim,
                self.build_head_windows(i, num_heads, local_attention_window, train_seq_len),
                qk_gain_init,
            )
    ...
    @staticmethod
    def build_head_windows(layer_idx: int, num_heads: int, local_attention_window: int, train_seq_len: int) -> list[int]:
        full_heads = min(layer_idx + 1, num_heads)
        restricted_heads = max(num_heads - full_heads, 0)
        layer_base_window = local_attention_window * (2 ** layer_idx)
        head_windows: list[int] = []
        for head_idx in range(restricted_heads):
            head_window = layer_base_window * (2 ** head_idx)
            head_windows.append(0 if head_window >= train_seq_len else head_window)
        return head_windows + [0] * full_heads

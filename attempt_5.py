...
Attempt 5 notes:
- Builds on attempt 4.
- Changes the default architecture from 4 KV heads to 8 KV heads.
- Removes the default paired sharing of K/V across query-head pairs so head roles are independent by default.
"""
...
# - 8 attention heads with 8 KV heads (independent K/V per query head) and 2x MLP expansion
...
class CausalSelfAttention(nn.Module):
    ...
    # - optional partial RoPE on a prefix of q and k
    # - causal masked SDPA
    def __init__(
        self,
        dim: int,
        ...
        if len(head_windows) != num_heads:
            raise ValueError("head_windows must provide one window per query head")
        if self.head_dim % 2 != 0:
            raise ValueError("head_dim must be even for RoPE")
        if partial_rope_dim < 0:
            raise ValueError("partial_rope_dim must be non-negative")
        if partial_rope_dim == 0:
            partial_rope_dim = self.head_dim
        if partial_rope_dim > self.head_dim:
            raise ValueError("partial_rope_dim must be <= head_dim")
        if partial_rope_dim % 2 != 0:
        ...
        self.head_windows = mx.array(head_windows, dtype=mx.int32)
        self.rope = nn.RoPE(self.partial_rope_dim, traditional=False, base=rope_base)
        self.scale = self.head_dim ** -0.5

    def apply_partial_rope(self, x: mx.array) -> mx.array:
        if self.partial_rope_dim == self.head_dim:
            return self.rope(x)
        x_rope = self.rope(x[..., : self.partial_rope_dim])
        x_pass = x[..., self.partial_rope_dim :]
        return mx.concatenate((x_rope, x_pass), axis=-1)

    def expand_kv_heads(self, x: mx.array) -> mx.array:
        if self.num_kv_heads == self.num_heads:
            return x
        # Non-default fallback for GQA/MQA variants. attempt_5 defaults to num_kv_heads=num_heads
        # so randomized local/global head roles do not share K/V representations in pairs.
        repeat = self.num_heads // self.num_kv_heads
        bsz, _, seqlen, width = x.shape
        x = x.reshape(bsz, self.num_kv_heads, 1, seqlen, width)
        x = mx.broadcast_to(x, (bsz, self.num_kv_heads, repeat, seqlen, width))
        return x.reshape(bsz, self.num_heads, seqlen, width)

    def build_head_mask(self, seqlen: int) -> mx.array:
        q_pos = mx.arange(seqlen, dtype=mx.int32)[:, None]
        k_pos = mx.arange(seqlen, dtype=mx.int32)[None, :]
        causal = k_pos <= q_pos
        window = self.head_windows[:, None, None]
        full_context = window <= 0
        within_window = k_pos[None, :, :] >= (q_pos[None, :, :] - (window - 1))
        return causal[None, :, :] & (full_context | within_window)

    def masked_attention(self, q: mx.array, k: mx.array, v: mx.array) -> mx.array:
        k = self.expand_kv_heads(k)
        v = self.expand_kv_heads(v)
        scores = (q.astype(mx.float32) * self.scale) @ k.astype(mx.float32).transpose(0, 1, 3, 2)
        allowed = self.build_head_mask(q.shape[2])[None, :, :, :]
        scores = mx.where(allowed, scores, mx.array(-1e9, dtype=scores.dtype))
        probs = mx.softmax(scores, axis=-1).astype(v.dtype)
        return probs @ v
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, partial_rope_dim, head_windows, qk_gain_init)
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 logit_chunk_tokens: int, logit_softcap: float, rope_base: float, partial_rope_dim: int, local_attention_window: int, tied_embed_init_std: float,
                 schedule_seed: int,
                 qk_gain_init: float):
        super().__init__()
        ...
        if local_attention_window <= 0:
            raise ValueError(f"local_attention_window must be positive, got {local_attention_window}")
        ...
        self.blocks = [
            Block(
                ...
                rope_base,
                partial_rope_dim,
                self.build_head_windows(i, num_layers, num_heads, local_attention_window, schedule_rng),
                qk_gain_init,
            )
    ...
    def build_head_windows(
        ...
        # Always preserve one local head, even in the deepest layers.
        full_heads = min(layer_idx + 1, num_heads - 1)
        restricted_heads = max(num_heads - full_heads, 0)
        head_windows = np.array([local_attention_window] * restricted_heads + [0] * full_heads, dtype=np.int32)
        schedule_rng.shuffle(head_windows)
        return head_windows.tolist()

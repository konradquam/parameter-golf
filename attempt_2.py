...
Attempt 2 notes:
- Builds on partial RoPE.
- Replaces fused causal attention with custom masked attention.
- Uses a per-layer head schedule where local heads shrink to a fixed local window and one additional head becomes full-context each layer.
"""
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        self.blocks = [
            Block(
                ...
                rope_base,
                partial_rope_dim,
                self.build_head_windows(i, num_layers, num_heads, local_attention_window),
                qk_gain_init,
            )
    ...
    @staticmethod
    def build_head_windows(layer_idx: int, num_layers: int, num_heads: int, local_attention_window: int) -> list[int]:
        full_heads = min(layer_idx + 1, num_heads)
        restricted_heads = max(num_heads - full_heads, 0)
        return [local_attention_window] * restricted_heads + [0] * full_heads

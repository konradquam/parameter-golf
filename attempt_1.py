...
Attempt 1 notes:
- Adds partial RoPE support via `PARTIAL_ROPE_DIM`.
- Applies RoPE only to a prefix of each Q/K head while leaving the remaining channels unchanged.
"""
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, partial_rope_dim, qk_gain_init)
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 logit_chunk_tokens: int, logit_softcap: float, rope_base: float, partial_rope_dim: int, tied_embed_init_std: float,
                 qk_gain_init: float):
        super().__init__()
        ...
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, partial_rope_dim, qk_gain_init)
